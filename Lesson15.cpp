...
bool Lesson15::init()
{
	...
	keyboardListener->onKeyPressed = CC_CALLBACK_2(Lesson15::onKeyPressed, this);
	keyboardListener->onKeyReleased = CC_CALLBACK_2(Lesson15::onKeyReleased, this);
	...
	_client->on("friendMove", CC_CALLBACK_2(Lesson15::onFriendMove, this));
...
void Lesson15::onMouseDown(Event* event) {
	...
	rapidjson::Document eventData = createEventData("haolv",
		this->mySprite->getPosition().x, this->mySprite->getPosition().y);

	const char* strJsonData = GetJsonText(eventData);

	log("strJson: %s", strJsonData);

    _client->emit("SayHello", strJsonData);
}



void Lesson15::onKeyPressed(EventKeyboard::KeyCode keyCode, Event* event) {
...
void Lesson15::onKeyReleased(EventKeyboard::KeyCode keyCode, Event* event) {
	auto newDirection = direction;
...
void Lesson15::onFriendMove(cocos2d::network::SIOClient* client, const std::string& data) {
